...
sdp.simple_eye(signal[100*samples_per_symbol:], samples_per_symbol*3, ntrace, t_sample, "{}Gbps 4-PAM Signal".format(data_rate/1e9), 100)

signal_ctle = sp.signal.convolve(signal,h_ctle)

sdp.simple_eye(signal_ctle[100*samples_per_symbol:], samples_per_symbol*3, ntrace, t_sample, "{}Gbps 4-PAM Signal with CTLE".format(data_rate/1e9),100)
...
sdp.channel_coefficients(h_pulse[:t.size],t,samples_per_symbol,2,4,100,"channel coefficients")
h = sdp.channel_coefficients(h_pulse_ctle[:t.size],t,samples_per_symbol,2,4,100, "channel + CTLE coefficients")
#%%
#h /= h.max()
#print('h: ',h)
...
sdp.simple_eye(RX.signal[int(1000.5*samples_per_symbol):], samples_per_symbol*3, ntrace, t_sample, "Eye Diagram with CTLE and FFE",100)

RX.pam4_DFE(w_dfe)

sdp.simple_eye(RX.signal[int(1000.5*samples_per_symbol):], samples_per_symbol*3,  ntrace, t_sample, "Eye Diagram with CTLE, FFE, and DFE",100)

...
cdr_zeta = 0.707
wn = 2 * np.pi * cdr_bn
cdr_alpha = 2 * cdr_zeta * wn
cdr_beta = wn**2
#cdr_beta = 1e-5  # CDR filter coefficient

cdr_pos0= 0

for cdr_pos0 in range(0,4,4):
    ...
    count_symbol=0  
    cdr_filter_state = 0.0  # Initialize CDR filter state
    cdr_phase = 0.0  # Initialize CDR phase
    ...
    ted_errors = np.zeros(N_symbols) # Store errors for analysis/debug
    ...
    filter_out_errors = np.zeros(N_symbols)
    cdr_phase_errors = np.zeros(N_symbols) # Store CDR phase errors for analysis/debug
    print(f"CDR alpha: {cdr_alpha}, CDR beta: {cdr_beta}, CDR bandwidth: {cdr_bn}, CDR zeta: {cdr_zeta}")
    for k in range(num_ffe_post_taps, N_symbols-num_ffe_taps):
        cdr_pos = int(0.5*cdr_phase/np.pi* samples_per_symbol + cdr_pos0)
        ...
        previous_decision = ffe_output_symbols[k-1] if k > 0 else 0  # Previous decision for TED calculation
        previous_sample = received_signal[k-1] if k > 0 else 0  # Previous sample for TED calculation
        ted_error = slicer_input * previous_decision - previous_sample * current_decision

        cdr_filter_state += cdr_beta * ted_error
        filter_output = cdr_alpha * ted_error + cdr_filter_state
        
        # Update VCO phase
        cdr_phase += filter_output
        cdr_phase = np.mod(cdr_phase, 2 * np.pi)
        ...
        ted_errors[k] = ted_error # Store for analysis/debug
        ffe_output_symbols[k] = current_decision
        filter_out_errors[k] = filter_output
        cdr_phase_errors[k] = cdr_pos
...
time_axis3 = np.arange(N_symbols)
plt.figure(figsize=(12, 8))
plt.plot(time_axis3, filter_out_errors, '.-', label='CDR LPF Out', color='orange', alpha=0.5)
plt.figure(figsize=(12, 8))
plt.plot(time_axis3, cdr_phase_errors, 'o-', label='CDR Phase', color='blue',alpha=0.5)
plt.figure(figsize=(12, 8))
plt.plot(time_axis3, ted_errors, '--', label='TED', color='yellow',alpha=0.5)
#annotation_text = "Channel Taps: " + str(channel_taps) + "FFE Taps: " + str(ffe_taps) 
#plt.title(annotation_text)
plt.title('CDR  Convergence')
